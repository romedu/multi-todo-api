...
exports.checkPermission = async (req, res, next) => {
   try {
      ...
      isUserOwner = `${currentList.creator && currentList.creator._id}` === `${user.id}`;
...
exports.checkIfFolderOwner = async (req, res, next) => {
   try {
      ...
      const currentListFolder = await Folder.findOne({ name: body.folderName }).populate("creator").exec();

      if (!currentListFolder) throw errorHandler(404, "Not Found")
      else {
         const isUserFolderOwner = `${currentListFolder.creator.id}` === `${user.id}`;

         if (!isUserFolderOwner) throw errorHandler(401, "You are not authorized to proceed");
