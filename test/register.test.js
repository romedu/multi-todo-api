...
describe("Register route", () => {
   ...
   describe("Post request", () => {
      ...
      describe("Sending valid data", () => {
         ...
         beforeAll(async () => {
            response = await request(app).post(baseUrl).send(validTestUserData);
         });
      ...
      describe("Sending invalid data", () => {
         describe("Sending one of the inputs invalid", () => {
            describe("Using a taken username", () => {
               it("should return a status of 409", async done => {
                  const response = await request(app).post(baseUrl).send(validTestUserData);
         ...
         describe("Passing both inputs invalid", () => {
            ...
            beforeAll(async () => {
               response = await request(app).post(baseUrl).send(invalidTestUserData);
            });
